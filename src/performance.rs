...
mod tests {
    ...
    #[test]
    fn actual_fps_counts_picture_updates_without_discarding_presentations() {
        let session = PerformanceMonitor::new("high");
        let track = session.for_video_track(0);
        let other = session.for_video_track(1);
        let mut timing = RenderedFrameTiming {
            is_new_picture: Some(true),
            width: 1920,
            height: 1080,
            decoded_at: Instant::now(),
            local: Duration::ZERO,
            assembly: Duration::ZERO,
            input_queue: Duration::ZERO,
            decode_pipeline: Duration::ZERO,
            surface_transfer: Duration::ZERO,
            present_wait: Duration::ZERO,
            render_queue: Duration::ZERO,
            sender_capture_at: None,
            sender_capture: None,
            sender_encode: None,
            sender_pacer: None,
            sender_total: None,
        ...
        for index in 0..44 {
            timing.is_new_picture = Some(index < 28);
            track.record_rendered_frame(timing);
        }
        mutex_lock(&track.inner.rates).sampled_at = Instant::now() - Duration::from_secs(1);
        let stats = track.build_snapshot();
        assert_eq!(stats.total_rendered_frames, 44);
        assert_eq!(stats.total_actual_rendered_frames, 28);
        assert_eq!(stats.total_marked_rendered_frames, 44);
        assert!((stats.actual_fps / stats.render_fps - 28.0 / 44.0).abs() < 0.0001);

        // A newly shown renderer counts its first presentation, even if the
        // source marks it unchanged. Subsequent repeats must not count.
        other.record_rendered_frame(timing);
        other.record_rendered_frame(timing);
        assert_eq!(other.build_snapshot().total_actual_rendered_frames, 1);
        track.pause_presentation();
        track.record_rendered_frame(timing);
        track.record_rendered_frame(timing);
        timing.is_new_picture = None;
        track.record_rendered_frame(timing);
        let stats = track.build_snapshot();
        assert_eq!(stats.total_rendered_frames, 47);
        assert_eq!(stats.total_actual_rendered_frames, 30);
        assert_eq!(stats.total_marked_rendered_frames, 46);
        assert_eq!(other.build_snapshot().total_rendered_frames, 2);
    }
}
