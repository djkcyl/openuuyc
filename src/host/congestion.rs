...
impl Controller {
    ...
    pub fn automatic_rates(&self) -> (u32, u32, f64) {
        // One ordinary desktop observer: the extended allocation raises its
        // ceiling to max_probe. It is not the last discrete probe measurement.
        (
            self.bounds.allocation(self.target, true),
            self.bounds.allocation(
                self.core
                    .bounded_target_rate()
                    .bps_or(0)
    ...
    pub fn media_allocation(&self) -> u32 {
        self.bounds.allocation(self.target, false)
    }
