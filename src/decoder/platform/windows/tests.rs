...
#[test]
#[ignore = "offline hardware benchmark requires an explicit captured corpus JSON"]
fn benchmark_captured_hardware_input() {
    use sha2::{Digest, Sha256};
    use std::time::Instant;
    let path = std::path::PathBuf::from(
        std::env::var_os("OPENUUYC_VIDEO_BENCH_CASE").expect("select captured case"),
    );
    let case: serde_json::Value = serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
    assert_eq!(
        case["bframes"].as_u64(),
        Some(0),
        "timing corpus must have no output reordering"
    );
    let data = std::fs::read(path.with_extension("annexb")).unwrap();
    let mut offset = 0;
    let units: Vec<_> = case["au_lengths"]
        .as_array()
        .unwrap()
        .iter()
        .map(|length| {
    ...
    let codec = if case["codec"].as_str() == Some("hevc") {
        CodecKind::Hevc
    } else {
        CodecKind::H264
    };
    let device = device();
    let mut config = config(codec, Some(&device));
    config.width = case["width"].as_u64().unwrap() as u32;
    config.height = case["height"].as_u64().unwrap() as u32;
    let expected = case["frames"].as_array().unwrap();
    assert_eq!(units.len(), expected.len());
    let mut samples = Vec::new();
    for round in 0..4 {
        let mut decoder = WindowsVideoDecoder::open(&config).unwrap();
        for (index, unit) in units.iter().enumerate() {
            let packet = packet(unit, index);
            ...
            // Synchronous readback fences the previous GPU work equally for both
            // builds. Neither readback nor hashing is charged as submit latency.
            let pixels = gpu_pixels(&frame);
            if round == 0 {
                assert_eq!(
                    format!("{:x}", Sha256::digest(&pixels)),
                    expected[index]["sha256"].as_str().unwrap()
                );
            }
            let release = Instant::now();
            drop(frame);
            if round != 0 {
                samples.push((submit + release.elapsed()).as_secs_f64() * 1000.0);
            }
        }
    }
    ...
    println!(
        "HARDWARE_BENCH {}",
        serde_json::json!({
            "case":case["name"], "decoder":"rust-dxva11",
            "frames":units.len(), "rounds":3,
            "mean_submit_ms":samples.iter().sum::<f64>() / samples.len() as f64,
            "p95_submit_ms":samples[(samples.len() * 95 / 100).min(samples.len()-1)],
            "pixel_check":"all input frames checked in warmup",
            "scope":"submit, poll, release; synchronous GPU readback excluded"
        })
    );
}
