...
pub(crate) const DEFAULT_CHECK_INTERVAL: Duration = Duration::from_millis(48);
...
pub(crate) const UU_RECEIVING_TIMEOUT: Duration = Duration::from_millis(2_500);
pub(crate) const UU_BACKUP_CONNECTION_PING_INTERVAL: Duration = Duration::from_secs(25);
pub(crate) const UU_RECEIVING_SWITCHING_DELAY: Duration = Duration::from_secs(1);
pub(crate) const UU_REGATHER_FAILED_NETWORKS_INTERVAL: Duration = Duration::from_secs(300);
pub(crate) const UU_STRONG_PING_INTERVAL: Duration = Duration::from_millis(480);
pub(crate) const UU_WEAK_PING_INTERVAL: Duration = Duration::from_millis(48);
pub(crate) const UU_WEAK_OR_STABILIZING_PING_INTERVAL: Duration = Duration::from_millis(900);
pub(crate) const UU_STABLE_WRITABLE_PING_INTERVAL: Duration = Duration::from_millis(2_500);
pub(crate) const UU_UNWRITABLE_TIMEOUT: Duration = Duration::from_secs(5);
pub(crate) const UU_UNWRITABLE_MIN_CHECKS: u16 = 5;
pub(crate) const UU_MIN_PINGS_AT_WEAK_INTERVAL: u64 = 3;
pub(crate) const UU_INACTIVE_TIMEOUT: Duration = Duration::from_secs(15);
...
pub struct AgentConfig {
    ...
    /// UU BasicIceController network preference. `None` is the official
    /// default (network preference disabled); when present the value is the
    /// adapter type to prefer (Ethernet=1, Wi-Fi=2, cellular=4, VPN=8, ...).
    pub network_preference: Option<u32>,

    /// UU VPN preference mode: 1/3 prefer VPN, 2/4 avoid VPN, other values
    /// leave VPN as a cost tie-breaker.
    pub vpn_preference: u32,

    /// Official `presume_writable_when_fully_relayed` switch. The shipped UU
    /// viewer leaves this disabled and the read-only client does the same.
    pub presume_writable_when_fully_relayed: bool,

    /// Keep ICE gathering open after an allocation pass.  UU enables this so
    /// failed-network regathering can surface new candidates without ending
