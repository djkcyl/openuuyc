...
impl AgentInternal {
    ...
    pub(crate) async fn get_candidate_pairs_stats(&self) -> Vec<CandidatePairStats> {
        ...
        for cp in &*checklist {
            let stat = CandidatePairStats {
                timestamp: Instant::now(),
                local_candidate_id: cp.local_candidate().id(),
                remote_candidate_id: cp.remote.id(),
                state: cp.state.load(Ordering::SeqCst).into(),
                nominated: cp.nominated.load(Ordering::SeqCst),
                requests_sent: cp.requests_sent.load(Ordering::Relaxed),
                responses_received: cp.responses_received.load(Ordering::Relaxed),
                total_round_trip_time: cp.total_round_trip_micros.load(Ordering::Relaxed) as f64
                    / 1_000_000.0,
                current_round_trip_time: cp.current_round_trip_micros.load(Ordering::Relaxed)
                    as f64
                    / 1_000_000.0,
    ...
    pub(crate) async fn get_local_candidates_stats(&self) -> Vec<CandidateStats> {
        ...
        for candidate in self.local_candidates.lock().await.values().flatten() {
            candidates.insert(candidate.id(), candidate.clone());
        }
        // Private punch Ports and retained old Connections can be selected
        // without being present in the current SDP advertisement list.
        for pair in self.agent_conn.checklist.lock().await.iter() {
            let local = pair.local_candidate();
            candidates.insert(local.id(), local);
        }
    ...
    pub(crate) async fn get_remote_candidates_stats(&self) -> Vec<CandidateStats> {
        ...
        // Private prflx and retained old-generation Connections do not belong
        // to the current signaled-candidate list, but selected-pair stats still
        // refer to them.
        for pair in self.agent_conn.checklist.lock().await.iter() {
            candidates.insert(pair.remote.id(), pair.remote.clone());
        }
        candidates
            .into_values()
            .map(|candidate| CandidateStats {
                timestamp: Instant::now(),
                id: candidate.id(),
