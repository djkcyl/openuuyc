...
mod gather_owner;
mod local_port;
pub(crate) mod network_catalog;
mod punch;
mod shared_socket;
pub(crate) mod tcp_port;
...
pub(crate) struct BindingRequest {
    ...
    pub(crate) credential_generation: u32,
    pub(crate) local_candidate_id: String,
...
struct ChanReceivers {
    chan_state_rx: mpsc::UnboundedReceiver<ConnectionState>,
...
pub struct Agent {
    ...
    pub(crate) gathering_generation: Arc<AtomicU32>,
    network_catalog: Arc<Mutex<network_catalog::NetworkCatalog>>,
    gather_sessions: std::sync::Mutex<Vec<Arc<allocator::AllocationSession>>>,
    pub(crate) candidate_types: Arc<SyncRwLock<Vec<CandidateType>>>,
    pub(crate) urls: Arc<SyncRwLock<Vec<Url>>>,
    pub(crate) network_types: Vec<NetworkType>,
    pub(crate) continual_gathering: bool,
    pub(crate) gather_owner: Arc<gather_owner::GatherOwner>,
}

impl Drop for Agent {
    fn drop(&mut self) {
        self.internal.abandon();
        self.gather_owner.begin_close();
    }
}
...
impl Agent {
    ...
    pub async fn new(config: AgentConfig) -> Result<Self> {
        ...
        let agent = Self {
            ...
            gathering_generation: Arc::new(AtomicU32::new(0)),
            network_catalog: Arc::new(Mutex::new(network_catalog::NetworkCatalog::default())),
            gather_sessions: std::sync::Mutex::new(Vec::new()),
            candidate_types: Arc::new(SyncRwLock::new(candidate_types)),
            urls: Arc::new(SyncRwLock::new(config.urls.clone())),
            network_types: config.network_types.clone(),
            continual_gathering: config.continual_gathering,
            gather_owner: Arc::new(gather_owner::GatherOwner::default()),
        };
    ...
    pub fn add_remote_candidate(&self, c: &Arc<dyn Candidate + Send + Sync>) -> Result<()> {
        if self.internal.agent_conn.done.load(Ordering::SeqCst) {
            return Err(Error::ErrClosed);
        }
        let description: Arc<dyn Candidate + Send + Sync> = Arc::new(
            crate::candidate::candidate_base::remote_description_copy(&**c),
        );
        let c = &description;
        ...
        if c.candidate_type() == CandidateType::Host && c.address().ends_with(".local") {
            ...
            self.internal.spawn_io(async move {
    ...
    pub async fn get_local_candidates(&self) -> Result<Vec<Arc<dyn Candidate + Send + Sync>>> {
        let username = self.internal.ufrag_pwd.lock().await.local_ufrag.clone();
        self.get_local_candidates_for_username(&username).await
    }

    pub async fn get_local_candidates_for_username(
        &self,
        username: &str,
        ...
        let generations = self
            ...
            .await
            .iter()
            .filter(|(_, credentials)| credentials.username == username)
...

        {
            ...
            for candidates in local_candidates.values() {
                for candidate in candidates {
                    if generations.contains(&candidate.generation()) {
    ...
    pub async fn get_candidate_username_fragment(
        &self,
        candidate: &Arc<dyn Candidate + Send + Sync>,
    ) -> Option<String> {
        if !self
            .internal
            .candidate_is_currently_published(candidate)
            .await
        {
            return None;
        }
    ...
    pub async fn close(&self) -> Result<()> {
        if self.internal.agent_conn.done.swap(true, Ordering::SeqCst) {
            return Err(Error::ErrClosed);
        }
        self.internal.begin_close_io();
        self.gather_owner.begin_close();
        for session in self
            .gather_sessions
            .lock()
            .expect("gather sessions poisoned")
            .iter()
        {
            session.stop.cancel();
        }
        let resources = self.gather_owner.finish().await;
    ...
    pub fn is_controlling(&self) -> bool {
        self.internal.is_controlling.load(Ordering::Acquire)
    }
    ...
    /// Reconfigures which candidates and ICE servers the next gathering
    /// generation may use. Existing candidates and the selected connection are
    /// deliberately retained until a newer generation is nominated.
    pub fn set_gathering_configuration(&self, candidate_types: Vec<CandidateType>, urls: Vec<Url>) {
        *self
            .candidate_types
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = if candidate_types.is_empty() {
            default_candidate_types()
        } else {
            candidate_types
        };
        *self
            .urls
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = urls;
    }
    ...
    pub async fn restart(&self, mut ufrag: String, mut pwd: String) -> Result<()> {
        ...
        let first_generation = self.internal.ufrag_pwd.lock().await.local_ufrag.is_empty();
        let generation = if first_generation {
            0
        } else {
            self.gathering_generation.fetch_add(1, Ordering::SeqCst) + 1
        };
        self.gathering_generation
            .store(generation, Ordering::SeqCst);
        self.internal
            .active_credential_generation
            .store(generation, Ordering::SeqCst);
        self.internal.retire_older_ports(generation).await;
        let snapshot = {
            let mut ufrag_pwd = self.internal.ufrag_pwd.lock().await;
            ufrag_pwd.local_ufrag = ufrag;
            ufrag_pwd.local_pwd = pwd;
            // A locally initiated restart keeps the current remote
            // credentials until the restart answer installs the new ones.
            IceCredentials {
                username: ufrag_pwd.local_ufrag.clone(),
                password: ufrag_pwd.local_pwd.clone(),
            }
        };
        self.internal
            .credential_generations
        ...
        // libwebrtc keeps the old checklist and selected connection alive
        // while gathering the new generation. Pending transactions are also
        // retained and validated against their generation credentials.
        self.internal.start().await;

        // Restart is used by NewAgent. Accept/Connect should be used to move to checking
        // for new Agents
        if self.internal.agent_conn.get_selected_pair().is_none()
            && self.internal.connection_state.load(Ordering::SeqCst) != ConnectionState::New as u8
        {
            self.internal
    ...
    pub fn gather_candidates(&self) -> Result<()> {
        ...
        self.gathering_state
            .store(GatheringState::Gathering as u8, Ordering::Release);
        let session =
            allocator::AllocationSession::new(self.gathering_generation.load(Ordering::Acquire));
        self.gather_sessions
            .lock()
            .expect("gather sessions poisoned")
            .push(session.clone());
        let params = GatherCandidatesInternalParams {
            owner: self.gather_owner.clone(),
            udp_network: self.udp_network.clone(),
            candidate_types: self
                .candidate_types
                .read()
                .expect("candidate types poisoned")
                .clone(),
            urls: self.urls.read().expect("ICE URLs poisoned").clone(),
            ...
            ext_ip_mapper: self.ext_ip_mapper.clone(),
            agent_internal: self.internal.clone(),
            gathering_state: self.gathering_state.clone(),
            chan_candidate_tx: self.internal.chan_candidate_tx.clone(),
            include_loopback: self.include_loopback,
            continual_gathering: self.continual_gathering,
            network_catalog: self.network_catalog.clone(),
        };
        self.internal.punch.register(&params, &session);
        self.gather_owner.spawn(async move {
            allocator::run(params, session).await;
        });
