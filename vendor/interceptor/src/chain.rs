...
impl Interceptor for Chain {
    ...
    /// Propagate RTCP-only updates without replacing the receive chain.
    async fn update_remote_rtcp(
        &self,
        ssrc: u32,
        parameters: crate::stream_info::ReceiveRtcpParameters,
    ) {
        for interceptor in &self.interceptors {
            interceptor.update_remote_rtcp(ssrc, parameters).await;
        }
    }
