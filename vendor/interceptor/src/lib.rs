...
/// Packet metadata. Arrival belongs to the datagram, not to the task which
/// eventually drains its SSRC. Keep it typed (also on 32-bit targets).
#[derive(Default, Debug, Clone, PartialEq, Eq)]
pub struct Attributes {
    pub rtp_received_at: Option<std::time::Instant>,
...
pub trait Interceptor {
    ...
    /// Apply RTCP-only renegotiation without rebinding RTP or clearing loss,
    /// sequence, jitter, and sender-report state.
    async fn update_remote_rtcp(
        &self,
        _ssrc: u32,
        _parameters: stream_info::ReceiveRtcpParameters,
    ) {
    }
