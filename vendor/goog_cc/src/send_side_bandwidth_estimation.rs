...
pub struct SendSideBandwidthEstimation {
    ...
    // UU 4.41: supplied by the separately evaluated link-pressure detector.
    random_loss_no_backoff: bool,
}
...
impl SendSideBandwidthEstimation {
    pub(crate) fn set_random_loss_no_backoff(&mut self, random_loss: bool) {
        self.random_loss_no_backoff = random_loss;
    }
    ...
    pub fn on_route_change(&mut self) {
        self.random_loss_no_backoff = false;
    ...
    /// UU's lower-limit statistic is the target saved after estimator bounds,
    /// before the separate receiver-limit view (T53BABA -> T53BBB4).
    pub fn bounded_target_rate(&self) -> DataRate {
        self.current_target
    }
    ...
    pub fn update_estimate(&mut self, at_time: Timestamp) {
        ...
        if time_since_loss_packet_report < 1.2 * Self::MAX_RTCP_FEEDBACK_INTERVAL {
            ...
            if self.current_target < self.bitrate_threshold || loss <= self.low_loss_threshold {
                ...
                if loss <= self.high_loss_threshold || (self.random_loss_no_backoff && loss <= 0.2)
                {
