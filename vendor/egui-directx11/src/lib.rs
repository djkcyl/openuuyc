...
pub struct Renderer {
    ...
    prepared_meshes: Vec<PreparedMesh>,
...
impl Renderer {
    ...
    pub fn render(
        ...
        self.prepare(device_context, egui_ctx, egui_output, frame_size)?;
        self.render_prepared(device_context, render_target)
    }

    /// Upload a new egui output and retain its immutable GPU geometry for
    /// subsequent compositions. This avoids tessellating and creating buffers
    /// again when the same overlay is drawn over a new video frame.
    pub fn prepare(
        &mut self,
        device_context: &ID3D11DeviceContext,
        egui_ctx: &egui::Context,
        ...
        // egui frees textures AFTER the frame using them. Retain them while
        // prepared geometry can still be rendered; release on replacement.
        let mut retired = TexturesDelta::default();
        retired.free.extend(self.pending_texture_frees.drain(..));
        self.texture_pool.update(device_context, retired)?;
        self.pending_texture_frees
            .extend(egui_output.textures_delta.free.drain());
        self.texture_pool
            .update(device_context, egui_output.textures_delta)?;
        if egui_output.shapes.is_empty() {
            self.prepared_meshes.clear();
            return Ok(());
        }
        ...
        for (index, mesh) in meshes.iter().enumerate() {
            let vertex_bytes = mem::size_of_val(mesh.vtx.as_slice());
            let index_bytes = mem::size_of_val(mesh.idx.as_slice());
            if index == self.prepared_meshes.len() {
                let vertex_capacity = buffer_capacity(vertex_bytes);
                let index_capacity = buffer_capacity(index_bytes);
                self.prepared_meshes.push(PreparedMesh {
            ...
            let prepared = &mut self.prepared_meshes[index];
            if prepared.vertex_capacity < vertex_bytes {
                prepared.vertex_capacity = buffer_capacity(vertex_bytes);
                prepared.vertex_buffer = Self::create_dynamic_buffer(
                    &self.device,
                    D3D11_BIND_VERTEX_BUFFER,
                    prepared.vertex_capacity,
                )?;
            }
            if prepared.index_capacity < index_bytes {
                prepared.index_capacity = buffer_capacity(index_bytes);
                prepared.index_buffer = Self::create_dynamic_buffer(
                    &self.device,
                    D3D11_BIND_INDEX_BUFFER,
                    prepared.index_capacity,
                )?;
            }
            Self::upload_dynamic_buffer(
                device_context,
                &prepared.vertex_buffer,
                mesh.vtx.as_ptr().cast(),
                vertex_bytes,
            )?;
            Self::upload_dynamic_buffer(
                device_context,
                &prepared.index_buffer,
                mesh.idx.as_ptr().cast(),
                index_bytes,
            )?;
            prepared.index_count = mesh.idx.len() as u32;
            prepared.tex = mesh.tex;
            prepared.clip_rect = mesh.clip_rect;
        }
        self.prepared_meshes.truncate(meshes.len());
        Ok(())
    }

    /// Draw the most recently prepared output without rebuilding its meshes.
    pub fn render_prepared(
        &mut self,
        device_context: &ID3D11DeviceContext,
        render_target: &ID3D11RenderTargetView,
    ) -> Result<()> {
        if self.prepared_meshes.is_empty() {
            return Ok(());
        }
        let frame_size = Self::get_render_target_size(render_target)?;
        self.setup(device_context, render_target, frame_size);
        for mesh in &self.prepared_meshes {
            Self::draw_prepared_mesh(device_context, &self.texture_pool, mesh);
        }
        Ok(())
    }
...
impl Renderer {
    ...
    fn upload_dynamic_buffer(
        ...
        unsafe { context.Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, Some(&raw mut mapped)) }?;
        unsafe {
            core::ptr::copy_nonoverlapping(data.cast::<u8>(), mapped.pData.cast::<u8>(), size);
...
fn buffer_capacity(required: usize) -> usize {
