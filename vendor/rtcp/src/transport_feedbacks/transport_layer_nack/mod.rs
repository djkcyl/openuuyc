...
impl Marshal for TransportLayerNack {
    ...
    fn marshal_to(&self, mut buf: &mut [u8]) -> Result<usize, util::Error> {
        // RTCP's length field is 16 bits. The five-bit feedback format is
        // fixed to Generic NACK and does not limit the number of NACK pairs.
        if self.nacks.len() + TLN_LENGTH > u16::MAX as usize {
...
pub fn nack_pairs_from_sequence_numbers(seq_nos: &[u16]) -> Vec<NackPair> {
    ...
    for &seq in seq_nos.iter().skip(1) {
        ...
        // Input follows RTP sequence order, which can cross 65535 -> 0.
