...
pub enum Error {
    ...
    ErrOneAllocateOnly,
    #[error("TURN Allocate requires a fresh socket (437)")]
    AllocationMismatch,
    #[error("TURN Allocate redirected to {target} (300)")]
    AllocationRedirect {
        target: net::SocketAddr,
        realm: Option<String>,
        nonce: Option<String>,
    },
    #[error("TURN Allocate failed ({code}): {reason}")]
    AllocationRejected { code: u16, reason: String },
