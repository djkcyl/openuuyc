...
impl Net {
    ...
    pub fn new(config: Option<NetConfig>) -> Self {
        if let Some(config) = config {
            ...
            Net::Ifs(Self::native_interfaces())
        }
    }

    fn native_interfaces_result() -> Result<Vec<Interface>> {
        let interfaces = ifaces::ifaces()?;

        let mut m: HashMap<String, (Vec<IpNet>, u32)> = HashMap::new();
        for iface in interfaces {
            if let Some((addrs, adapter_type)) = m.get_mut(&iface.name) {
                if let Some(addr) = iface.addr {
                    if let Ok(inet) = Interface::convert(addr, iface.mask) {
                        addrs.push(inet);
                    }
                }
                *adapter_type = (*adapter_type).max(iface.adapter_type);
            } else if let Some(addr) = iface.addr {
                if let Ok(inet) = Interface::convert(addr, iface.mask) {
                    m.insert(iface.name, (vec![inet], iface.adapter_type));
                }
            }
        }

        let mut ifs = vec![];
        for (name, (addrs, adapter_type)) in m.into_iter() {
            ifs.push(Interface::new_with_adapter_type(name, addrs, adapter_type));
        }

        Ok(ifs)
    }

    fn native_interfaces() -> Vec<Interface> {
        match Self::native_interfaces_result() {
            Ok(interfaces) => interfaces,
            Err(error) => {
                log::warn!("native interface enumeration failed: {error}");
                Vec::new()
            }
        }
    }

    /// An enumeration error is not an empty network snapshot. Stateful ICE
    /// managers must keep their previous snapshot when this operation fails.
    pub async fn try_get_interfaces(&self) -> Result<Vec<Interface>> {
        match self {
            Net::VNet(vnet) => Ok(vnet.lock().await.get_interfaces().to_vec()),
            Net::Ifs(_) => Self::native_interfaces_result(),
        }
    }
    ...
    pub async fn get_interfaces(&self) -> Vec<Interface> {
        match self {
            ...
            Net::Ifs(_) => Self::native_interfaces(),
        }
    }
    ...
    pub async fn get_interface(&self, ifc_name: &str) -> Option<Interface> {
        match self {
            ...
            Net::Ifs(_) => {
                for ifc in Self::native_interfaces() {
                    if ifc.name == ifc_name {
                        return Some(ifc.clone());
                    }
                }
