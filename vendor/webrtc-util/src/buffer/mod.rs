...
struct BufferInternal {
    ...
    limit_count: usize,
    limit_size: usize,
    // Same lock and packet lifetime as the byte ring, including discarded
    // short reads. This is metadata, not an additional packet queue.
    timestamps: VecDeque<Option<Instant>>,
}
...
impl Buffer {
    ...
    pub async fn write(&self, packet: &[u8]) -> Result<usize> {
        self.write_with_timestamp(packet, None).await
    }
    ...
    pub async fn write_with_timestamp(
        ...
        b.count += 1;
        b.timestamps.push_back(received_at);
    ...
    pub async fn read(&self, packet: &mut [u8], duration: Option<Duration>) -> Result<usize> {
        Ok(self.read_with_timestamp(packet, duration).await?.0)
    }
    ...
    ) -> Result<(usize, Option<Instant>)> {
        loop {
            // Capture notify_waiters' generation before checking closed under
            // the lock. Otherwise close between unlock and notified() is lost.
            // notify_one still retains the existing single-reader data permit.
            let ready = self.notify.notified();
            {
                ...
                if b.head != b.tail {
                    ...
                    let received_at = b
                        .timestamps
                        .pop_front()
                        .expect("packet metadata matches byte ring");

                    if copied < count {
                        return Err(Error::ErrBufferShort);
                    }
                    return Ok((copied, received_at));
                } else {
            ...
            if let Some(d) = duration {
                timeout(d, ready).await.map_err(|_| Error::ErrTimeout)?;
            } else {
                ready.await;
            }
        }
