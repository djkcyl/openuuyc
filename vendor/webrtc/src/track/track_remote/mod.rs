...
impl TrackRemote {
    ...
    /// Returns the negotiated numeric id for an RTP header-extension URI.
    /// Inbound packets only carry this id, not the URI from SDP.
    ...
    /// Subscribe to decrypted RTP in the exact order accepted by the shared
    /// SRTP session. Normal per-SSRC readers must still be drained so their
    /// interceptor chains continue to run.
    pub async fn subscribe_incoming_rtp(
        &self,
    ...
    /// Returns `(media, rtx, fec)` SSRCs for this negotiated track. A zero
    /// auxiliary SSRC means that stream was not negotiated.
    pub async fn associated_ssrcs(&self) -> Result<(SSRC, SSRC, SSRC)> {
        self.receiver
            .as_ref()
            .and_then(Weak::upgrade)
            .ok_or(Error::ErrClosedPipe)?
            .associated_ssrcs(self.tid)
            .await
    }
    ...
    /// Reads an RTP packet from the associated RTX repair stream.
    pub async fn read_rtx(&self) -> Result<(rtp::packet::Packet, Attributes)> {
        let mut buffer = vec![0u8; self.receive_mtu];
        let receiver = self
            .receiver
            .as_ref()
            .and_then(Weak::upgrade)
            .ok_or(Error::ErrClosedPipe)?;
        receiver.read_rtx(&mut buffer, self.tid).await
    }

    /// Reads an RTP packet from the associated UU RS-FEC repair stream.
    pub async fn read_fec(&self) -> Result<(rtp::packet::Packet, Attributes)> {
        let mut buffer = vec![0u8; self.receive_mtu];
        let receiver = self
            .receiver
            .as_ref()
            .and_then(Weak::upgrade)
            .ok_or(Error::ErrClosedPipe)?;
        receiver.read_fec(&mut buffer, self.tid).await
    }
