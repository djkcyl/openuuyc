            ...
            match attr.key.as_str() {
                ATTR_KEY_SSRCGROUP => {
                    if let Some(value) = &attr.value {
                        ...
                        } else if split[0] == "FEC-FR-RS" && split.len() == 3 {
                            let Ok(base_ssrc) = split[1].parse::<u32>() else {
                                continue;
                            };
                            let Ok(fec_ssrc) = split[2].parse::<u32>() else {
                                continue;
                            };
                            fec_repair_flows.insert(fec_ssrc, base_ssrc);
                            filter_track_with_ssrc(&mut tracks_in_media_section, fec_ssrc as SSRC);
                        }
                    }
                }
                ...
                ATTR_KEY_SSRC => {
                    if let Some(value) = &attr.value {
                        ...
                        if rtx_repair_flows.contains_key(&ssrc)
                            || fec_repair_flows.contains_key(&ssrc)
                        {
                            continue; // This SSRC is an associated repair flow, ignore as a track.
                        }
    ...
    if let (Some(sd), Some(ice)) = (session_description, ice_gatherer) {
        ...
        if !parsed.media_descriptions.is_empty() {
            ...
            let username = m
                .attributes
                .iter()
                .chain(parsed.attributes.iter())
                .find(|attribute| attribute.key == "ice-ufrag")
                .and_then(|attribute| attribute.value.as_deref());
            let candidates = match username {
                Some(username) => match ice.get_local_candidates_for_username(username).await {
                    Ok(candidates) => candidates,
                    Err(_) => return Some(sd.clone()),
                },
                None => Vec::new(),
            };
            // Return the live advertisement set for this SDP's credentials.
            // Removed Ports must not survive merely because they were in the
            // original SDP; a new generation must not be appended to old SDP.
            m.attributes.retain(|attribute| {
                attribute.key != "candidate" && attribute.key != "end-of-candidates"
            });
...
pub(crate) async fn extract_ice_details(
    ...
    for candidate in &mut candidates {
        candidate.username_fragment = Some(remote_ufrag.to_owned());
    }
