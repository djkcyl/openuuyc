...
use interceptor::stream_info::{ReceiveRtcpParameters, RtcpMode};
...
impl PeerConnectionInternal {
    ...
    pub(super) async fn start_rtp(
        ...
        // Apply remote RTCP preference to existing modules too. Rebinding
        // their RTP readers here would erase sequence/loss/SR history.
        for transceiver in &current_transceivers {
            if let Some(media) = transceiver
                .mid()
                .and_then(|mid| get_by_mid(mid.as_str(), &remote_desc))
            {
                Self::configure_receive_rtcp(
                    transceiver,
                    media.attributes.iter().any(|a| a.key == "rtcp-rsize"),
                )
                .await;
            }
        }
    ...
    async fn configure_receive_rtcp(transceiver: &RTCRtpTransceiver, reduced_size: bool) {
        let sender = transceiver.sender().await;
        let local_ssrc = if sender.track().await.is_some() {
            sender
                .get_parameters()
                .await
                .encodings
        ...
        transceiver
            .receiver()
            .await
            .set_receive_rtcp_parameters(ReceiveRtcpParameters {
                mode: if reduced_size {
    ...
    async fn handle_incoming_rtp_stream(
        ...
        // A probed SSRC has no media-section owner yet. Enable its report
        // module only after MID/RID resolves to an actual receiver.
        stream_info.receiver_rtcp = Some(ReceiveRtcpParameters {
            mode: RtcpMode::Off,
            local_ssrc: None,
        });
                ...
                (stream_stats.into_iter().zip(track_infos)).filter_map(|(s, i)| s.map(|s| (s, i)))
            {
                ...
                collector.insert(
                    ...
                    crate::stats::StatsReportType::RemoteOutboundRTP(RemoteOutboundRTPStats {
                        ...
                        round_trip_time: remote_round_trip_time.map(|ms| ms / 1000.0),
                        total_round_trip_time: remote_total_round_trip_time / 1000.0,
                ...
                .filter_map(|(s, i)| s.map(|s| (s, i)))
            {
                ...
                collector.insert(
                    ...
                    StatsReportType::RemoteInboundRTP(RemoteInboundRTPStats {
                        ...
                        round_trip_time: remote_rtt_ms.map(|ms| ms / 1000.0),
                        total_round_trip_time: remote_total_rtt_ms / 1000.0,
