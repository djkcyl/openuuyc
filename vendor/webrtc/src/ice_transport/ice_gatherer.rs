...
impl RTCIceGatherer {
    ...
    pub(crate) async fn create_agent(&self) -> Result<()> {
        ...
        let mut config = ice::agent::agent_config::AgentConfig {
            ...
            // UU advertises numeric TURNS endpoints whose certificate is issued to
            // *.relay.nrd.nie.163.com; its native client uses the no-name-check policy.
            insecure_skip_verify: true,
            network_preference: self.setting_engine.candidates.network_preference,
            vpn_preference: self.setting_engine.candidates.vpn_preference,
            presume_writable_when_fully_relayed: self
                .setting_engine
                .candidates
                .presume_writable_when_fully_relayed,
            continual_gathering: self.setting_engine.candidates.continual_gathering,
    ...
    pub async fn gather(&self) -> Result<()> {
        ...
        if let Some(agent) = self.get_agent().await {
            ...
            let candidate_agent = Arc::downgrade(&agent);
            ...
            agent.on_candidate(Box::new(
                move |candidate: Option<Arc<dyn Candidate + Send + Sync>>| {
                    ...
                    let candidate_agent = candidate_agent.clone();

                    Box::pin(async move {
                        let Some(candidate_agent) = candidate_agent.upgrade() else {
                            return;
                        };
                        if RTCIceGathererState::from(state_clone.load(Ordering::SeqCst))
                        ...
                        if let Some(cand) = candidate {
                            if let Some(handler) = &*on_local_candidate_handler_clone.load() {
                                let mut candidate = RTCIceCandidate::from(&cand);
                                let mut f = handler.lock().await;
                                let Some(username) =
                                    candidate_agent.get_candidate_username_fragment(&cand).await
                                else {
                                    return;
                                };
                                candidate.username_fragment = Some(username);
    ...
    pub(crate) async fn set_gathering_configuration(
        ...
        if let Some(agent) = self.get_agent().await {
            let candidate_types = if policy == RTCIceTransportPolicy::Relay {
                vec![CandidateType::Relay]
            } else {
                Vec::new()
            };
            agent.set_gathering_configuration(candidate_types, urls);
            Ok(())
        } else {
    ...
    pub(crate) async fn get_local_candidates_for_username(
        &self,
        username: &str,
    ) -> Result<Vec<RTCIceCandidate>> {
        let candidates = if let Some(agent) = self.get_agent().await {
            agent.get_local_candidates_for_username(username).await?
        } else {
            Vec::new()
        };
        Ok(rtc_ice_candidates_from_ice_candidates(&candidates))
    }
