...
use interceptor::stream_info::{AssociatedStreamInfo, RTPHeaderExtension, ReceiveRtcpParameters};
...
pub struct RTPReceiverInternal {
    ...
    rtcp_parameters: std::sync::Mutex<ReceiveRtcpParameters>,
...
impl RTPReceiverInternal {
    pub(crate) async fn subscribe_incoming_rtp(
        &self,
    ) -> Result<broadcast::Receiver<srtp::session::IncomingRtpPacket>> {
        self.transport
            .get_srtp_session()
            .await
            .and_then(|session| session.subscribe_incoming_rtp())
            .ok_or(Error::ErrDtlsTransportNotStarted)
    }

    pub(crate) async fn associated_ssrcs(&self, tid: usize) -> Result<(SSRC, SSRC, SSRC)> {
        let tracks = self.tracks.read().await;
        let streams = tracks
            .iter()
            .find(|streams| streams.track.tid() == tid)
            .ok_or(Error::ErrRTPReceiverForRIDTrackStreamNotFound)?;
        let stream_ssrc =
            |stream: &TrackStream| stream.stream_info.as_ref().map_or(0, |info| info.ssrc);
        Ok((
            stream_ssrc(&streams.stream),
            stream_ssrc(&streams.repair_stream),
            stream_ssrc(&streams.fec_stream),
        ))
    }
    ...
    pub(crate) async fn read_rtx(
        &self,
        buffer: &mut [u8],
        tid: usize,
    ) -> Result<(rtp::packet::Packet, Attributes)> {
        let mut state = self.state_tx.subscribe();
        State::wait_for(&mut state, &[State::Started]).await?;
        loop {
            let repair_interceptor = {
                let tracks = self.tracks.read().await;
                tracks
                    .iter()
                    .find(|track| track.track.tid() == tid)
                    .and_then(|track| track.repair_stream.rtp_interceptor.clone())
            };
            if let Some(repair_interceptor) = repair_interceptor {
                let packet = repair_interceptor.read(buffer, &Attributes::new()).await?;
                return Ok(packet);
            }
            tokio::select! {
                changed = state.changed() => {
                    changed.map_err(|_| Error::ErrClosedPipe)?;
                    if *state.borrow() == State::Stopped {
                        return Err(Error::ErrClosedPipe);
                    }
                }
        ...
        loop {
            let fec_interceptor = {
                ...
                tracks
                    .iter()
                    .find(|track| track.track.tid() == tid)
                    .and_then(|track| track.fec_stream.rtp_interceptor.clone())
            };
            if let Some(fec_interceptor) = fec_interceptor {
                let packet = fec_interceptor.read(buffer, &Attributes::new()).await?;
                return Ok(packet);
            }
            tokio::select! {
                changed = state.changed() => {
...
impl RTCRtpReceiver {
    pub fn new(
        ...
        RTCRtpReceiver {
            ...
            internal: Arc::new(RTPReceiverInternal {
                ...
                rtcp_parameters: std::sync::Mutex::new(ReceiveRtcpParameters::default()),
            }),
        }
    }
    ...
    pub(crate) async fn set_receive_rtcp_parameters(&self, parameters: ReceiveRtcpParameters) {
        *self
            .internal
            .rtcp_parameters
            .lock()
            .unwrap_or_else(|e| e.into_inner()) = parameters;
        let mut tracks = self.internal.tracks.write().await;
        for track in &mut *tracks {
            for stream in [
                &mut track.stream,
                &mut track.repair_stream,
                &mut track.fec_stream,
            ] {
                if let Some(info) = &mut stream.stream_info {
                    info.receiver_rtcp = Some(parameters);
                    self.internal
                        .interceptor
                        .update_remote_rtcp(info.ssrc, parameters)
                        .await;
                }
            }
        }
    ...
    async fn configure_probed_rtcp(&self, stream: &mut TrackStream) {
        if let Some(info) = &mut stream.stream_info {
            let parameters = *self
                .internal
                .rtcp_parameters
        ...
        for encoding in &parameters.encodings {
            let (stream_info, rtp_read_stream, rtp_interceptor, rtcp_read_stream, rtcp_interceptor) =
                if encoding.ssrc != 0 {
                    ...
                    stream_info.payload_clock_rates = global_params
                        .codecs
                        .iter()
                        .map(|codec| (codec.payload_type, codec.capability.clock_rate))
                        .collect();
                    stream_info.receiver_rtcp = Some(
                        *self
                            .internal
                            .rtcp_parameters
                            .lock()
            ...
            let fec_ssrc = encoding.fec.ssrc;
            if fec_ssrc != 0 {
                let fec_codec = global_params
                    .codecs
                    .iter()
                    .find(|candidate| {
                        candidate
                ...
                let fec_info = AssociatedStreamInfo {
                    ssrc: encoding.ssrc,
                ...
                let mut stream_info = create_stream_info(
                    "".to_owned(),
                    fec_ssrc,
                    fec_codec.payload_type,
                    fec_codec.capability,
                    &global_params.header_extensions,
                    Some(fec_info),
                );
                stream_info.receiver_rtcp = Some(
                ...
                let (rtp_read_stream, rtp_interceptor, rtcp_read_stream, rtcp_interceptor) = self
                    .internal
                    .transport
                    .streams_for_ssrc(fec_ssrc, &stream_info, &interceptor)
                    .await?;
                self.receive_for_fec(
                    fec_ssrc,
                    TrackStream {
                        stream_info: Some(stream_info),
                        rtp_read_stream: Some(rtp_read_stream),
    ...
    pub(crate) async fn start(&self, incoming: &TrackDetails) {
        ...
        for (i, encoding) in encodings.iter_mut().enumerate() {
            ...
            encoding.fec.ssrc = incoming.fec_ssrc;
        }
    ...
    pub async fn stop(&self) -> Result<()> {
        ...
        if was_ever_started {
            ...
            for t in &*tracks {
                ...
                if let Some(fec_rtcp_read_stream) = &t.fec_stream.rtcp_read_stream {
                    if let Err(err) = fec_rtcp_read_stream.close().await {
                        errs.push(err);
                    }
                }

                if let Some(fec_rtp_read_stream) = &t.fec_stream.rtp_read_stream {
                    if let Err(err) = fec_rtp_read_stream.close().await {
                        errs.push(err);
                    }
                }
                ...
                if let Some(fec_stream_info) = &t.fec_stream.stream_info {
                    self.internal
                        .interceptor
                        .unbind_remote_stream(fec_stream_info)
                        .await;
                }
            }
        }
    ...
    pub(crate) async fn read_rtx(
        ...
        self.internal.read_rtx(buffer, tid).await
    }
    ...
    pub(crate) async fn read_fec(
        ...
        self.internal.read_fec(buffer, tid).await
    }
    ...
    pub(crate) async fn receive_for_rid(
        ...
        for t in &mut *tracks {
            if *t.track.rid() == rid {
                self.configure_probed_rtcp(&mut stream).await;
    ...
    pub(crate) async fn receive_for_rtx(
        ...
        ssrc: SSRC,
        rsid: String,
        mut repair_stream: TrackStream,
    ) -> Result<()> {
        let mut tracks = self.internal.tracks.write().await;
        let l = tracks.len();
        for t in &mut *tracks {
            if (ssrc != 0 && l == 1) || t.track.rid() == rsid {
                self.configure_probed_rtcp(&mut repair_stream).await;
    ...
    pub(crate) async fn receive_for_fec(
        &self,
        ssrc: SSRC,
        mut fec_stream: TrackStream,
    ) -> Result<()> {
        let mut tracks = self.internal.tracks.write().await;
        let count = tracks.len();
        for track in &mut *tracks {
            if ssrc != 0 && count == 1 {
                self.configure_probed_rtcp(&mut fec_stream).await;
                track.fec_stream = fec_stream;
                return Ok(());
            }
        }
        Err(Error::ErrRTPReceiverForRIDTrackStreamNotFound)
