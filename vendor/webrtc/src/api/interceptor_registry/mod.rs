...
pub fn configure_twcc_receiver_only(
    ...
    configure_twcc_receiver_with_builder(registry, media_engine, Receiver::builder())
}

/// Configure the same negotiated feedback with an owner's interval policy.
pub fn configure_twcc_receiver_with_builder(
    mut registry: Registry,
    media_engine: &mut MediaEngine,
    receiver: interceptor::twcc::receiver::ReceiverBuilder,
) -> Result<Registry> {
    media_engine.register_feedback(
        RTCPFeedback {
            typ: TYPE_RTCP_FB_TRANSPORT_CC.to_owned(),
