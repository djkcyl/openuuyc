...
// IDs 15..=255 are valid in RFC 8285's two-byte form. OpenUUYC offers
// extmap-allow-mixed; silently omitting the fifteenth negotiated extension
// would drop UU's new-picture marker while leaving the rest of SDP valid.
const VALID_EXT_IDS: Range<isize> = 1..256;
        ...
        let ext = {
                ...
                .find(|ext| ext.uri == extension.uri)
            {
                ...
                None => {
                    ...
                    if self.header_extensions.len() >= VALID_EXT_IDS.len() {
